const pathOne = "serverConfig";
const pathTwo = "wareBusiness";
const pathThree = "basicConfig";
const url = $request.url;
const body = $response.body;
const $tool = tool();

if (url.indexOf(pathOne) != -1) {
...
if (url.indexOf(pathTwo) != -1) {
  ...
  request_history_price(shareUrl)
    .then((data) => {
      if (data.errno == -1) throw new Error("Oops");
...
function priceSummary(data) {
  let summary = `🌨 当前: ${data.CurrentPrice}${getSpace(8)}最低: ${
    data.LowestPrice
  } (${data.LowestDate})`;
